...
/**
  ...
  * @retval         遥控器数据指针
  */
const RC_ctrl_t * get_remote_control_point(void)    //const RC_ctrl_t *get_remote_control_point(void)
{
    return &rc_ctrl;
}
...
static void sbus_to_rc(volatile const uint8_t *sbus_buf, RC_ctrl_t *rc_ctrl)
{
    ...
    rc_ctrl->rc.ch[0] = ((sbus_buf[1] >> 3) | (sbus_buf[2] << 5)) & 0x07ff;        //!< Channel 0
    rc_ctrl->rc.ch[1] = (sbus_buf[0] | (sbus_buf[1] << 8)) & 0x07ff; //!< Channel 1
...
//	//传统遥控器s[0]取反
//	if(RC_SW_UP == rc_ctrl->rc.s[0])
//	{
//		rc_ctrl->rc.s[0] = RC_SW_DOWN;
//	}
//	else if(RC_SW_DOWN == rc_ctrl->rc.s[0])
//	{
...
void sbus_to_usart1(uint8_t *sbus)
{
    ...
    memcpy(usart_tx_buf + 1, sbus, 18);
	  usart_tx_buf[0] = 0xAA;
	  usart_tx_buf[1] = 0x31;//mode 31能量机关 30装甲板
	  usart_tx_buf[2] = 0x31;//color 31 红（自己） 30蓝
	  usart_tx_buf[3] = 0x30;
	  usart_tx_buf[4] = 0x30;
	  usart_tx_buf[5] = 0x30;
	  usart_tx_buf[6] = 0xbb;
