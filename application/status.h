...
// --- B. 摩擦轮与发射准备状态 (4-9位) ---
#define FLAG_L_CHARGE_REACHED (1 << 4)  // 左蓄力电机到位
#define FLAG_R_CHARGE_REACHED (1 << 5)  // 右蓄力电机到位
#define FLAG_L_REBOUND_REACHED (1 << 6) // 左反弹电机到位
#define FLAG_R_REBOUND_REACHED (1 << 7) // 右反弹电机到位
#define FLAG_GIMBAL_AIMED (1 << 8)      // 云台瞄准完成

// --- C. 动作完成标志 (10-15位) ---
#define FLAG_DART_DROPPED (1 << 10) // 电磁铁已断电，飞镖已掉入扳机
#define FLAG_FIRED (1 << 11)        // 扳机已释放，飞镖已射出
#define FLAG_STEP_DONE (1 << 12)    // 本发流程全部结束
...
#define DART_CLEAR_BIT(idx, flag) (DartSys.statusFlags[idx] &= ~(flag))
