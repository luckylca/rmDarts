...
/**
  ...
  */
/**
  * @brief          test任务,检测任务，时刻检测各个设备的状态，如果异常则蜂鸣器报警。
  * @param[in]      pvParameters: NULL
  * @retval         none
  */
void test_task(void const * argument)
{
    ...
    while(1)
    {
        ...
        //find error
        //发现错误
        for(error_num = DBUS_TOE; error_num <=ERROR_LIST_LENGHT; error_num++)
        {
					  
            if(error_list_test_local[error_num].error_exist&&( error_num==DBUS_TOE  || error_num==CHASSIS_MOTOR1_TOE || error_num==CHASSIS_MOTOR2_TOE))
            {
                error = 1;
                break;
            }
