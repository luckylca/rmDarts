...
osThreadId RM_Cilent_UI_handle;
/* USER CODE END PTD */
...
//		osThreadDef(autoTASK,auto_task,osPriorityNormal,0,1024);
//		auto_task_handle = osThreadCreate(osThread(autoTASK),NULL);//自动发射
...
//    battery_voltage_handle = osThreadCreate(osThread(BATTERY_VOLTAGE), NULL);
    ...
    servo_task_handle = osThreadCreate(osThread(SERVO), NULL);
	    //UI任务
//    osThreadDef(RM_Cilent_UI, UI_Task, osPriorityNormal, 0, 128);
//    testHandle = osThreadCreate(osThread(RM_Cilent_UI), NULL);
