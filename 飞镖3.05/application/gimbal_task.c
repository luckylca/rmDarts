...
#define FRC_PID_KP  10000.0f    //15000.0f
#define FRC_PID_KI 10.0f
#define FRC_PID_KD 0.0f
#define FRC_PID_MAX_OUT 1000.0f//16000.0f
#define FRC_PID_MAX_IOUT 2000.0f

extern uint8_t USART1_RX_DATA[5];
fp32 pitch_turn, yaw_turn,distance;
int16_t found_flag;//视觉数据解析
extern int sj_flag;

static fp32 FRC_PID[3];
static pid_type_def frcpid;
jpid frc1pid;
jpid frc2pid;
pid_type_def virppid;
pid_type_def virypid;
//extern fp32 PVIR_PID[3];
//extern fp32 YVIR_PID[3];
//extern pid_type_def virppid;
//extern pid_type_def virypid;
enum color
{
   blue=0x00,
	 red
...
uint8_t yaw_buchang=0x00;
uint8_t pitch_buchang=0x00;
fp32 vpturn,vyturn;
...
static void gimbal_motor_absolute_angleyaw_control(gimbal_motor_t *gimbal_motor);
/**
...
static void gimbal_absolute_angleyaw_limit(gimbal_motor_t *gimbal_motor, fp32 add);
/**
...
//static void track(gimbal_control_t *track_t);
...
uint8_t USART1_TX_DATA[7];

//motor current 
//发送的电机电流
int16_t yaw_can_set_current = 0, pitch_can_set_current = 0, shoot_can_set_current = 0;
int16_t frc1_can_set_current = 0, frc2_can_set_current = 0;
/**
...
void gimbal_task(void const *pvParameters)
{
    ...
    shoot_init();
	  ...
	  FRC_PID[0]=FRC_PID_KP;
	  FRC_PID[1]=FRC_PID_KI;
	  FRC_PID[2]=FRC_PID_KD;
	
	  PVIR_PID[0]=PVIR_PID_KP;
	  PVIR_PID[1]=PVIR_PID_KI;
	  PVIR_PID[2]=PVIR_PID_KD;
	
	
	  YVIR_PID[0]=YVIR_PID_KP;
	  YVIR_PID[1]=YVIR_PID_KI;
	  YVIR_PID[2]=YVIR_PID_KD;
		
		
	  PID_init(&frcpid,PID_POSITION,FRC_PID,FRC_PID_MAX_OUT,FRC_PID_MAX_IOUT);
	  PID_init(&virppid,PID_POSITION,PVIR_PID,PVIR_PID_MAX_OUT,PVIR_PID_MAX_IOUT);
	  PID_init(&virypid,PID_POSITION,YVIR_PID,YVIR_PID_MAX_OUT,YVIR_PID_MAX_IOUT);

//填充视觉消息数组
		int8_t i = 0;
		USART1_TX_DATA[0] = 0xAA;
		USART1_TX_DATA[6] = 0xBB;
		for(i = 1; i < 6; i++)
		{
			USART1_TX_DATA[i] = i;
		}
    ...
    {
				////根据视觉信息转动云台
				pitch_turn = (int16_t)((USART1_RX_DATA[0] << 8) | USART1_RX_DATA[1]);
				pitch_turn = pitch_turn * 90.0f / 32767.0f / 57.3f;//转换为rad
				yaw_turn = (int16_t)((USART1_RX_DATA[2] << 8) | USART1_RX_DATA[3]);
				yaw_turn = yaw_turn * 90.0f / 32767.0f / 57.3f;//转换为rad
				found_flag = USART1_RX_DATA[4];
				//distance= USART1_RX_DATA[5];
				if (found_flag == 1)
				{  
					vpturn = pitch_turn;
					vyturn =yaw_turn;
...
//		
		...
		//与上位机通信
		//HAL_UART_Transmit(&huart1,USART1_TX_DATA,TX_size, 1);
        ...
        if (!(toe_is_error(YAW_GIMBAL_MOTOR_TOE) && toe_is_error(PITCH_GIMBAL_MOTOR_TOE) && toe_is_error(TRIGGER_MOTOR_TOE)))
        {
            if (toe_is_error(DBUS_TOE))
            {
                CAN_cmd_gimbal(0, 0, 0, 0);
							  frc1pid.out=PID_calc(&frcpid,frc1pid.ref,0.0f);
					      frc2pid.out=PID_calc(&frcpid,frc2pid.ref,0.0f);
							  frc1_can_set_current=(int16_t)(frc1pid.out);
				        frc2_can_set_current=(int16_t)(frc2pid.out);
							  //CAN_cmd_gimbal_frc(0, 0, 0);
            }
            else
            {
                //CAN_cmd_gimbal(yaw_can_set_current , pitch_can_set_current , 0, 0);
				
							  //CAN_cmd_gimbal_frc(frc1_can_set_current, frc2_can_set_current, 0);
            }
...
static void gimbal_set_control(gimbal_control_t *set_control)
{
    ...
    {
        //gyro模式下，陀螺仪角度控制
        gimbal_absolute_angleyaw_limit(&set_control->gimbal_yaw_motor, add_yaw_angle);
    }
...
static void gimbal_absolute_angleyaw_limit(gimbal_motor_t *gimbal_motor, fp32 add)
{
    ...
    //relative angle + angle error + add_angle > max_relative angle
    //云台相对角度+ 误差角度 + 新增角度 如果大于 最大机械角度
    if (gimbal_motor->absolute_angle_set+ add > PI)
    {
			if(add>0.0f)
			{
        gimbal_motor->absolute_angle_set=gimbal_motor->absolute_angle_set + add-2*PI;
			}
    }
    else if (gimbal_motor->relative_angle + bias_angle + add < -PI)
    {
			if(add<0.0f)
			{
        gimbal_motor->absolute_angle_set=gimbal_motor->absolute_angle_set + add+2*PI;
		...
		{
			gimbal_motor->absolute_angle_set=gimbal_motor->absolute_angle_set+add;
		}
    angle_set = rad_format(gimbal_motor->absolute_angle_set);
...
static void gimbal_control_loop(gimbal_control_t *control_loop)
{
    ...
    {
        gimbal_motor_absolute_angleyaw_control(&control_loop->gimbal_yaw_motor);
    }
...
static void gimbal_motor_absolute_angle_control(gimbal_motor_t *gimbal_motor)
{
   ...
   // gimbal_motor->motor_gyro_set = gimbal_PID_calc(&gimbal_motor->gimbal_motor_absolute_angle_pid, gimbal_motor->absolute_angle, gimbal_motor->absolute_angle_set + vpturn, gimbal_motor->motor_gyro);
...
static void gimbal_motor_absolute_angleyaw_control(gimbal_motor_t *gimbal_motor)
{
    ...
    //角度环，速度环串级pid调试
		fp32 ang_set=gimbal_motor->absolute_angle_set;
		fp32 ang=gimbal_motor->absolute_angle;
		if(fabs(ang_set-ang)>PI)
		{
			if(ang_set>0)
				ang_set-=2*PI;
			else
		...
		gimbal_motor->motor_gyro_set = gimbal_PID_calc(&gimbal_motor->gimbal_motor_absolute_angle_pid, ang, ang_set , gimbal_motor->motor_gyro);
   // gimbal_motor->motor_gyro_set = gimbal_PID_calc(&gimbal_motor->gimbal_motor_absolute_angle_pid, ang, ang_set + vyturn, gimbal_motor->motor_gyro);
    gimbal_motor->current_set = PID_calc(&gimbal_motor->gimbal_motor_gyro_pid, gimbal_motor->motor_gyro, gimbal_motor->motor_gyro_set);
    //控制值赋值
    gimbal_motor->given_current = (int16_t)(gimbal_motor->current_set);
}
