...
//static
static	void chassis_rotate_set_control(fp32 *vx_set, fp32 *vy_set, fp32 *wz_set, chassis_move_t *chassis_move_rc_to_vector);
...
{   
    ...
    {
			// 开关为中
        chassis_behaviour_mode = CHASSIS_ROTA ;
    }
    ...
    {	
		...
		else if (chassis_behaviour_mode == CHASSIS_ROTATE)
    {
			// 不知道干嘛的
        chassis_move_mode->chassis_mode = CHASSIS_VECTOR_ROTATE;
    }
		else if (chassis_behaviour_mode == CHASSIS_ROTA)
    {
			// 飞镖
        chassis_move_mode->chassis_mode = CHASSIS_VECTOR_DOUBLE_SHOOT;
    }
}
...
void chassis_behaviour_control_set(fp32 *vx_set, fp32 *vy_set, fp32 *angle_set, chassis_move_t *chassis_move_rc_to_vector)
{
    ...
    {
        chassis_rotate_set_control(vx_set, vy_set, angle_set, chassis_move_rc_to_vector);
    }
		// 飞镖
		else if (chassis_behaviour_mode == CHASSIS_ROTA)
    {
...
static void chassis_infantry_follow_gimbal_yaw_control(fp32 *vx_set, fp32 *vy_set, fp32 *angle_set, chassis_move_t *chassis_move_rc_to_vector)
{
    ...
    //judge if swing
    //判断是否要摇摆
    if (chassis_move_rc_to_vector->chassis_RC->key.v & SWING_KEY)//改
    {
        if (swing_flag == 0)
        {
            swing_flag = 1;
...
{   rspeed=5.5;
	...
	{
		if(chassis_move_rc_to_vector->chassis_RC->key.v & KEY_PRESSED_OFFSET_V)
		{ rspeed+=2;
			if(rspeed>7.5)
				rspeed=7.5;
		}
		else if(chassis_move_rc_to_vector->chassis_RC->key.v & KEY_PRESSED_OFFSET_B)
		{ rspeed-=2;
			if(rspeed<3.5)
				rspeed=3.5;
		}
	}
	                                                //last=chassis_move_rc_to_vector->chassis_RC->key.v & CHASSIS_FRONT_KEY;}
	...
	//else rx=0;
		...
		//chassis_rc_to_control_vector(vx_set, vy_set, chassis_move_rc_to_vector);
    sita = chassis_move_rc_to_vector->chassis_yaw_motor->relative_angle;
//	ry =( chassis_move_rc_to_vector->chassis_RC->rc.ch[1]*0.001);                              //*CHASSIS_OPEN_RC_SCALE;
//    rx = chassis_move_rc_to_vector->chassis_RC->rc.ch[0]*0.001;                              //*CHASSIS_OPEN_RC_SCALE;
    *vx_set = ( -(-ry)*arm_cos_f32(sita) - (-rx)*arm_sin_f32(sita) ); 
	*vy_set = ( -(-ry)*arm_sin_f32(sita) + (-rx)*arm_cos_f32(sita) );
    *wz_set =rspeed;//rspeed;// 7.5;
...
//    }
//		
//     fp32 sita = chassis_move_rc_to_vector->chassis_yaw_motor->relative_angle;
//		 fp32   ry = chassis_move_rc_to_vector->chassis_RC->rc.ch[1]*0.001;//*CHASSIS_OPEN_RC_SCALE;
//		 fp32   rx = chassis_move_rc_to_vector->chassis_RC->rc.ch[0]*0.001;//*CHASSIS_OPEN_RC_SCALE;
//    *vx_set = ( -(-ry)*arm_cos_f32(sita) - (-rx)*arm_sin_f32(sita) );
//		*vy_set = ( -(-ry)*arm_sin_f32(sita) + (-rx)*arm_cos_f32(sita) );
//    *wz_set = 7.5;
// 
